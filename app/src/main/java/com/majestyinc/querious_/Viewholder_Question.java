...
public class Viewholder_Question extends RecyclerView.ViewHolder {
    ...
    ImageView imageView;
    TextView time_result, name_result, question_result, replybtn, replybtn1;
    ImageButton fvrt_btn, deletebtn;
    DatabaseReference favouriteref,repliesref;
    ...
    public void RepliesChecker(final String postkey) {

       TextView replies = itemView.findViewById(R.id.replies_no);



        repliesref = database.getReference("AllQuestions").child(postkey).child("Answers");
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
              ...
              int  replycount = (int) snapshot.getChildrenCount();
                if (snapshot.getChildrenCount() == 1) {
                    replies.setText(Integer.toString(replycount) + " reply");
                } else {
                    replies.setText(Integer.toString(replycount) + " replies");
                }
            }
